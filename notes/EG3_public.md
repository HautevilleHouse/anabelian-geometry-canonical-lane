...
In-paper anchor: `paper/ANABELIAN_GEOMETRY_PREPRINT.md` (`ANA_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of admissible reconstruction data from fundamental groups and sections through a multi-lane anabelian super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.

## Objects

- near-failure class: normalized candidate failures inside the declared admissible class.
- compactness modulus: `0.8038585209003215`.
...
`ANA_G3` closes when `0.8038585209003215` provides compactness/no-collapse control: normalized near-failure families stay precompact and restart spacing does not collapse.
This is the first-failure capture contribution to `M_ANA`.
...
If near-failure extraction and no-Zeno spacing hold, then `ANA_G3` prevents hidden drift before rigidity is applied.

## Current Instantiation

- gate: `ANA_G3`
- artifact key: `0.8038585209003215`
