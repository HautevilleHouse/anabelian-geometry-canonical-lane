...
In-paper anchor: `paper/ANABELIAN_GEOMETRY_PREPRINT.md` (`ANA_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving persistence of admissible reconstruction data from fundamental groups and sections through a multi-lane anabelian super-architecture`.
...
- projected core: the response sector controlled by `1.094116`.
...
`ANA_G1` closes when `1.094116` satisfies the response-floor requirement: projected response has the strict floor required by the main preprint.
This is the protected-core contribution to the strict margin `M_ANA`.
...
Payload: check that all quantities used by `1.094116` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `1.094116` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `1.094116`, then `ANA_G1` supplies the projected/protected-core input to `M_ANA`.

## Current Instantiation

- gate: `ANA_G1`
- artifact key: `1.094116`
